            ...
            if i >= pydevd_constants.PYDEVD_CONTAINER_RANDOM_ACCESS_MAX_ITEMS:
                ret.append(
                    (
                        TOO_LARGE_ATTR,
                        TOO_LARGE_MSG
                        % (pydevd_constants.PYDEVD_CONTAINER_RANDOM_ACCESS_MAX_ITEMS,),
                        None,
                    )
                )
                break
...
class MoreItemsRange:
    ...
    def __eq__(self, o):
        return (
            isinstance(o, MoreItemsRange)
            and self.value is o.value
            and self.from_i == o.from_i
            and self.to_i == o.to_i
        )
...
class TupleResolver:  # to enumerate tuples and lists
    def resolve(self, var, attribute):
        ...
        try:
            ...
            if attribute == "more":
                return MoreItems(
                    var, pydevd_constants.PYDEVD_CONTAINER_INITIAL_EXPANDED_ITEMS
                )
    ...
    def get_contents_debug_adapter_protocol(self, lst, fmt=None):
        ...
        for i, item in enumerate(lst):
            ...
            if i >= initial_expanded - 1:
                if (
                    lst_len - initial_expanded
                ) < pydevd_constants.PYDEVD_CONTAINER_BUCKET_SIZE:
