...
class Timer(object):
    ...
    def report_if_compute_repr_attr_slow(
        self, attrs_tab_separated, attr_name, attr_type
    ):
        self._report_slow(
            self._compute_repr_slow, attrs_tab_separated, attr_name, attr_type
        )
...
def import_attr_from_module(import_with_attr_access):
    if "." not in import_with_attr_access:
        ...
        raise ImportError(
            "Unable to import module with attr access: %s" % (import_with_attr_access,)
        )
    ...
    while True:
        try:
            ...
            if "." not in module_name:
                raise ImportError(
                    "Unable to import module with attr access: %s"
