...
class ValidationInfo(object):
    ...
    def verify_breakpoints(
        ...
        """
        valid_lines_frozenset, sorted_lines = self._collect_valid_lines_in_template(
            template
        )
    ...
    def verify_breakpoints_from_template_cached_lines(
        ...
        """
        cached = self._canonical_normalized_filename_to_last_template_lines.get(
            canonical_normalized_filename
        )
            ...
            if template_bp.verified_cache_key != valid_lines_frozenset:
                ...
                if not valid:
                    ...
                    if new_line >= 0 and new_line not in template_breakpoints_for_file:
                        ...
                        # location.
                        if (
                            template_bp.add_breakpoint_result.error_code
                            != PyDevdAPI.ADD_BREAKPOINT_NO_ERROR
                            and template_bp.add_breakpoint_result.translated_line
                            != new_line
                        ):
                            pydev_log.debug(
                        ...
                        if (
                            template_bp.add_breakpoint_result.error_code
                            != PyDevdAPI.ADD_BREAKPOINT_INVALID_LINE
                        ):
                            pydev_log.debug(
