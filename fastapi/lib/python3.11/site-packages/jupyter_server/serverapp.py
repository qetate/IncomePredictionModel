...
def _has_tornado_web_authenticated(method: t.Callable[..., t.Any]) -> bool:
    ...
    if hasattr(code, "co_qualname"):
        # new in 3.11
        return code.co_qualname.startswith(
            "authenticated"
...
class ServerApp(JupyterApp):
    ...
    def _allow_unauthenticated_access_default(self):
        if os.getenv(self._allow_unauthenticated_access_env):
            return os.environ[self._allow_unauthenticated_access_env].lower() in [
                "true",
                "yes",
            ]
        return True
    ...
    def _default_kernel_manager_class(
        self,
    ) -> t.Union[str, type[AsyncMappingKernelManager]]:
    ...
    def _default_kernel_spec_manager_class(
        self,
    ) -> t.Union[str, type[KernelSpecManager]]:
        if self.gateway_config.gateway_enabled:
