...
class SpecialPseudoPattern(SelectorPattern):
    ...
    def __init__(
        self,
        patterns: tuple[tuple[str, tuple[str, ...], str, type[SelectorPattern]], ...],
    ) -> None:
        """Initialize."""
...
class CSSParser:
    ...
    def parse_pseudo_class(
        ...
        if complex_pseudo and pseudo in PSEUDO_COMPLEX:
            ...
            raise SelectorSyntaxError(
                f"Invalid syntax for pseudo class '{pseudo}'", self.pattern, m.start(0)
            )
        else:
            raise SelectorSyntaxError(
    ...
    def parse_has_combinator(
        ...
        if combinator == COMMA_COMBINATOR:
            ...
            if has_selector:
                ...
                raise SelectorSyntaxError(
                    f"The multiple combinators at position {index}", self.pattern, index
                )
    ...
    def parse_selectors(
        ...
        if is_open and not closed:
            raise SelectorSyntaxError(
                f"Unclosed pseudo-class at position {index}", self.pattern, index
            )
