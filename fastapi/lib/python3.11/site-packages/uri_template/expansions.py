...
class Expansion:
        ...
        while index < len(value):
            ...
            if codepoint in legal:
                ...
                for byte in utf8:
                    output += (
                        "%"
                        + Charset.HEX_DIGIT[int(byte / 16)]
                        + Charset.HEX_DIGIT[byte % 16]
                    )
            index += 1
        return output
    ...
    def _uri_encode_name(self, name: (str | int)) -> str:
        ...
        return (
            self._encode(str(name), Charset.UNRESERVED + Charset.RESERVED, True)
            if (name)
            else ""
        )
    ...
    def _encode_str(
        ...
        if variable.max_length:
            ...
            return self._join(
                prefix, joiner, self._uri_encode_value(value[: variable.max_length])
            )
    ...
    def _encode_var(
        ...
        if isinstance(value, str):
            ...
            if len(value):
                encoded_items = [
                    self._encode_dict_item(
                        variable, name, key, value[key], delim, prefix, joiner, first
                    )
                    for key in value.keys()
                ]
                return delim.join(
                    [item for item in encoded_items if (item is not None)]
                )
            return None
        elif isinstance(value, collections.abc.Sequence):
            if len(value):
                encoded_items = [
                    self._encode_list_item(
                        variable, name, index, item, delim, prefix, joiner, first
                    )
            ...
            return self._encode_str(
                variable, name, str(value).lower(), prefix, joiner, first
            )
        else:
        ...
        for index, (expanded, missing) in enumerate(result):
            ...
            if expanded:
                output += (
                    (
                        self.output_prefix
                        if (first and (not self.trailing_joiner))
                        else ""
                    )
                ...
                output += (
                    (
                        ...
                        else (self.var_joiner if (not last) else "")
                    )
                    + "{"
...
class LabelExpansion(ExpressionExpansion):
    ...
    def _expand_var(self, variable: Variable, value: Any) -> str | None:
        ...
        return self._encode_var(
            variable,
            self._uri_encode_name(variable.name),
            value,
...
class PathStyleExpansion(ExpressionExpansion):
    ...
    def _encode_dict_item(
        ...
        return self._encode_var(
            variable,
            self._uri_encode_name(key) if (not variable.array) else "",
            item,
            delim,
            prefix,
            joiner,
    ...
    def _encode_list_item(
        ...
        if variable.array:
            if name:
                prefix = prefix + "[" + name + "]" if (prefix) else name
            return self._encode_var(
                variable, str(index), item, delim, prefix, joiner, False
            )
        return self._encode_var(
            variable,
            name,
    ...
    def _expand_var(self, variable: Variable, value: Any) -> str | None:
        ...
        if variable.explode:
            return self._encode_var(
                variable, self._uri_encode_name(variable.name), value, delim=";"
            )
        value = self._encode_var(
            variable, self._uri_encode_name(variable.name), value, delim=","
        )
        return (
            (self._uri_encode_name(variable.name) + "=" + value)
            if (value)
            else variable.name
        )
