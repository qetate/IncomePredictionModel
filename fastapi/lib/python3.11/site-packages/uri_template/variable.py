...
class Variable:
    ...
    def __init__(self, var_spec: str) -> None:
        ...
        while index < len(var_spec):
            ...
            if (
                ...
                raise VariableInvalidError(
                    var_spec
                    + ((":" + str(self.max_length)) if (self.max_length) else "")
    ...
    def __str__(self) -> str:
        ...
        return (
            self.name
            + (f"/{self.key}" if (self.key and (self.key != self.name)) else "")
            + (f":{self.max_length}" if (self.max_length) else "")
            + ("*" if (self.explode and not self.array) else "")
            + ("[]" if (self.array) else "")
